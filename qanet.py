...
class SelfAttention(nn.Module):
    def __init__(self,D,Nh=1,dropout=0.1):
        super().__init__()
    ...
    def forward(self, queries, mask):
        ...
        x = self.dot_product_attention(Q, K, V, mask = mask,dropout = self.dropout)
        return self.combine_last_two_dim(x.permute(0,2,1,3)).transpose(1, 2)

    def dot_product_attention(self, q, k ,v, bias = False, mask = None, dropout=0.1):
...
class Pointer(nn.Module):
    ...
    def forward(self, M1, M2, M3, mask):
        ...
        Y1 = util.masked_softmax(self.w1(X1).squeeze(), mask, log_softmax=True)
        Y2 = util.masked_softmax(self.w2(X2).squeeze(), mask, log_softmax=True)
        return Y1, Y2


class QANet(nn.Module):
    def __init__(self, D,  word_mat, char_mat, dropout=0.1):
        super().__init__()
        self.dropout=dropout
        ...
        self.emb = Embedding()
        self.emb_enc = EncoderBlock(D, conv_num=4, ch_num=D, k=7, dropout=dropout)
        self.cq_att = CQAttention()
        self.cq_resizer = Initialized_Conv1d(D * 4, D)
        self.model_enc_blks = nn.ModuleList([EncoderBlock(conv_num=2, ch_num=D, k=5, dropout=dropout) for _ in range(7)])
        self.out = Pointer(D)
